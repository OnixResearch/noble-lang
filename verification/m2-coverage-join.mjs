// M2 per-function coverage join (task 3.3).
//
// Parses the Aeneas-generated function surface of the M2 extraction subject
...
// (`verification/m2-coverage.json`). Used by verification/m2-coverage-gate.sh.
//
// Modes:
//   bun m2-coverage-join.mjs derive PROOF_ROOT        -> derived list (stdout)
//   bun m2-coverage-join.mjs emit   PROOF_ROOT        -> record skeleton (stdout)
//   bun m2-coverage-join.mjs check  PROOF_ROOT RECORD -> join report (stdout);
//     exit 0 = consistent, exit 1 = named violations
//   bun m2-coverage-join.mjs leanpayload PROOF_ROOT RECORD OUT-A OUT-B
...
//        prefix: NobleM2.Refinement.* -> OUT-B, everything else -> OUT-A)
//
...
if (import.meta.main) {
  ...
  if (mode === 'derive') {
    ...
    const record = {
      subject: 'M2 kernel extraction output (proofs/m2/NobleKernel)',
      record_version: 1,
    ...
    console.error('usage: m2-coverage-join.mjs derive|emit|check|leanpayload PROOF_ROOT [RECORD] [OUT-A] [OUT-B]');
    process.exit(2);
  }
}
