...
#[expect(
    ...
    reason = "Owner: noble-maintainers; forty_two builds the fixed quotation fixture; its acceptance, derivation count and limit boundaries are checked by its callers."
)]
fn forty_two() -> (
...
#[expect(
    tigerstyle::assertion_density,
    reason = "Owner: noble-maintainers; this negative control requires rejection via let-else and checks the hidden emission constraint; fixture size does not justify additional assertions."
)]
...
fn diagnostics_report_order_shape_and_truncation() -> Result<(), String> {
    ...
    #[expect(
        ...
        reason = "Owner: noble-maintainers; both diagnostic stacks are Vec<Ty> with nonzero-sized elements, so each length is at most isize::MAX and their sum fits usize; reassess if their representation changes."
    )]
    let diagnostic_entry_count = diagnostic.expected.len() + diagnostic.actual.len();
    assert!(diagnostic_entry_count <= 1);
    Ok(())
}
