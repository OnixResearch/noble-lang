<!-- Generated compatibility view. Edit .cairn/specs/evidence/spec.md instead. -->
...
The [language-workflow scenarios](conformance/language-workflow-cases.json) use design-lane labels, not new conformance profiles. Property-runner and documentation-harness tests retain their own subject and outcomes. A successful harness control does not mark its embedded Noble example as executed or proved.

The [worker scenarios](conformance/worker-cases.json) use `Worker-Design`, not a new language or concurrency profile. [WORKER-CONFORMANCE.md](WORKER-CONFORMANCE.md) separates pure worker requests, compiler-service effects, shell actions, and native retirement. Cancellation acknowledgement does not establish external completion. Round-trip expectations retain the canonical encoding gate.
...
[requirements.json](requirements.json) is a derived index of every numbered requirement in the registered normative documents. It records document ownership, planned evidence routes, scenario links, and open test-design gaps.
...
[Octet adoption cases](conformance/octet-adoption-cases.json) contain explicit stale-binding, tier-promotion, and incomplete-coverage controls. These are unexecuted designs. The document validator does not invoke Octet or a proof tool.
...
Proof obligations in [verification/obligations.json](verification/obligations.json) remain open. They name expected claims, not completed Lean declarations.
