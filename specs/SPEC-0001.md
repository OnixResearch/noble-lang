<!-- Generated compatibility view. Edit .cairn/specs/language/spec.md instead. -->
...
| Basis | [Preserved source chain](SOURCES.md) |
...
[DEVELOPER-EXPERIENCE.md](DEVELOPER-EXPERIENCE.md) adds scoped tooling and library contracts without changing the bootstrap expression grammar or ordinary execution.

[DECISIONS.md](DECISIONS.md) records the selected decisions. [CORE-BOOTSTRAP.md](CORE-BOOTSTRAP.md) limits the first implementation subset. [ROADMAP.md](ROADMAP.md) defines delivery order.
...
Lean 4 supplies the reference semantics and metatheory. Noble-owned production Rust targets Charon → Aeneas → Lean refinement. This route is mandatory for the entire semantic kernel, including resource-state decisions. Unsupported external effects require explicit boundaries, not silent verification exclusions. Verus is optional for reviewed non-kernel exceptions only. These tools operate at proof/build time, not as a second production Noble evaluator. See [IMPL-V001](VERIFICATION-TOOLCHAIN.md).
...
Resource-bearing programs remain possible through explicit resource inputs and ownership-preserving results. This example does not permit a live resource inside a capturable worker environment. [Worker conformance](WORKER-CONFORMANCE.md) defines a bounded application of these existing mechanisms.
...
Typed contracts and first-class proof companions are specified in [SPEC-V002](PROGRAM-CONTRACTS.md). Partial correctness, prefix safety and total correctness remain distinct claims. Proof-required invocation needs applicable evidence, satisfied preconditions, artifact correspondence, and independent host authorization. Verifying untrusted proof source is not an undocumented part of preparation.
...
[SPEC-BE001](BACKEND-EXPERIMENTS.md) requires an early comparison of WasmGC and managed linear memory. Typed function references, multi-value returns, and specialization are experiment candidates, not permission to discard stack inputs or observable recipes.
...
**W-WIT-05.** Owned WIT resources MUST lower to Noble move-only resources. Borrowed WIT resources MUST be scoped non-owning boundary values that cannot be captured, serialized, persisted, released as owned, or allowed to escape their declared borrow scope. [RESOURCE-ADAPTERS.md](RESOURCE-ADAPTERS.md) specifies the first synchronous adapter subset and rejects unsupported borrow patterns.
...
These contracts refine H-HOST-02 and H-OUTCOME-01/02. They add no implicit authorizer, receipt service, distributed retry policy, or proof requirement for ordinary programs. [Octet adoption](OCTET-ADOPTION.md) records their source and implementation gates.
...
The ownership transitions in [SPEC-R001](RESOURCE-ADAPTERS.md) govern cancellation/completion races for the selected async extension. These host records add no catch syntax, general handler, persistence guarantee, or automatic retry rule.
...
See [SPEC-V001](VERIFICATION.md), [SPEC-V002](PROGRAM-CONTRACTS.md), [IMPL-V001](VERIFICATION-TOOLCHAIN.md) and [DEC-N001](DECISIONS.md) for the selected obligations, tool roles, policy and open work.
