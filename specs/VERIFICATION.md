<!-- Generated compatibility view. Edit .cairn/specs/verification/spec.md instead. -->
...
This document specifies the claims to establish and the boundaries those claims must preserve. [VERIFICATION-TOOLCHAIN.md](VERIFICATION-TOOLCHAIN.md) requires an Aeneas-first Rust implementation, mandatory across the semantic kernel, with Lean 4 reference definitions and refinement proofs. Verus is available only for reviewed non-kernel exceptions. [PROGRAM-CONTRACTS.md](PROGRAM-CONTRACTS.md) specifies typed contracts, first-class evidence companions, and optional proof-required admission. [SPEC-0001.md](SPEC-0001.md) integrates these requirements. [SOURCES.md](SOURCES.md) records the inherited text and unavailable historical citation records.
...
The machine-readable backlog is [verification/obligations.json](verification/obligations.json). This ledger is newly transcribed from the PO and SO tables, not recovered from an older package. Each obligation remains open. Identifiers name theorem obligations, not implemented Lean declarations.
...
[Worker conformance](WORKER-CONFORMANCE.md) supplies test designs, not proofs of these claims. Scalar bootstrap evidence cannot close schema, package, async, or complete execution-profile coverage. An ideal semantic theorem does not establish a concrete interruption bound or eventual native retirement.
...
VT-OCTET-01–03 and EV-BIND-01/EV-TIER-01 add separate policy and evidence-admission obligations. Their results cannot close these semantic proof obligations by themselves. No obligation is accepted through the [adoption test designs](conformance/octet-adoption-cases.json).
...
These stages group assurance deliverables, not time estimates or mandatory serial dependencies. Pure V4 contract work can follow V2 without the V3 resource extension. [ROADMAP.md](ROADMAP.md) defines the MC1/MC2 dependency edges.
...
[CORE-BOOTSTRAP.md](CORE-BOOTSTRAP.md) scopes the first finite checker. [ROADMAP.md](ROADMAP.md) places extraction feasibility beside the checker and Wasm experiments. [EVIDENCE.md](EVIDENCE.md) prevents document validation from changing runtime or proof status.
