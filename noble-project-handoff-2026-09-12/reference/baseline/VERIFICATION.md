...
**V-CORE-01.** Ordinary `Program<S,T,e>`, `quote`, `compose`, `call`, and `reify` MUST retain their existing semantics. Ordinary execution MUST NOT require Lean, Aeneas, Verus, an SMT solver, or proof search to be installed or invoked. An admission service MAY check evidence separately under an explicit host policy.
...
| PO-07 | `quote`/`compose`/`call` execution laws and construction-effect separation | Lean 4 |
