...
| Verification extension | 0.1.0-draft.1, specified by SPEC-V001, SPEC-V002 and IMPL-V001; no completed proofs claimed |
| Safety extension | 0.1.0-draft.1, specified by SPEC-S001 and DEC-S001; no completed safety proof claimed |
| Component interface extension | 0.1.0-draft.1, specified by SPEC-W001 and DEC-W001; WIT/Component/WASI architecture selected, conformance not yet demonstrated |
...
**P-VERIFY-02.** Ordinary `quote`, `compose`, `run` and `reflect` MUST NOT invoke proof search. Reusable builder theorems may describe runtime-built programs, with their assumptions and instantiations checked separately by an explicit verification service when policy requires it.
...
**W-WIT-05.** Owned WIT resources MUST lower to Noble move-only resources. Borrowed WIT resources MUST be scoped non-owning boundary values that cannot be captured, serialized, persisted, released as owned, or allowed to escape their declared borrow scope.
...
**W-WASI-03.** Component Model native `async func`, `stream<T>`, and `future<T>` form Noble's standard cross-component async ABI. This requirement does not add `async`/`await` source forms or make asynchronous suspension imply concurrent execution.
...
| G-09 | For advertised verification claims: exact coverage, evidence/assumption ledger, pinned compatible tools, reproducible checking and explicit Rust/extraction/backend/host trust boundaries. |
